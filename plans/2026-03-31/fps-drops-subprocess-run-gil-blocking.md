...
# Fix FPS drops — subprocess.run GIL blocking
...
## Fichiers modifiés
- `TDDocker/python/td_docker/compose.py` — `_run_compose`
- `TDDocker/python/td_docker/container_manager.py` — si applicable

## Vérification
1. `cd TDDocker && python -m pytest python/tests/ -v` — tous les tests passent
2. Recharger modules dans TD
3. MCP `perf_monitor` : plus de spikes 300ms, FPS stable à 60 pendant polling
4. Up/Down fonctionnent toujours correctement
