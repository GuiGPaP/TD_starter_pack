...
## Étape immédiate : activer le mode bitmap obstacle

Le `select_mask` TOP est créé (pointe vers `/TDPretext/null_mask`). Le `layout_engine` a déjà `_build_segments_bitmap` et `_read_bitmap_spans` qui lisent `/TDPretextNative/select_mask`. Il faut juste :

1. Remettre le preset à `'editorial'` (mode cercles par défaut)
2. Vérifier que le switch `is_bitmap = (preset == 'displaced')` dans `onFrameEnd` fonctionne
3. Tester en passant le preset à `'displaced'` — le texte doit couler autour de la silhouette webcam

**Aucun code à écrire** — tout est déjà en place. Il suffit de basculer le preset.
