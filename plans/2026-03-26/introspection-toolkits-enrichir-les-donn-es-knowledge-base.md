...
### Étapes
1. **Introspection params** — Script `execute_python_script` sur les 49 opérateurs → `data/introspection/t3d.json`
2. **Enrichir `t3d.json`** — Mettre à jour le stub toolkit avec inventaire complet, `detectionPaths: ["/T3D_1_12_7"]`
3. **Pas de templates exemples** — Pas d'example loader. Les `.toe` ne peuvent pas être chargés dynamiquement. On skip les templates pour T3D.
