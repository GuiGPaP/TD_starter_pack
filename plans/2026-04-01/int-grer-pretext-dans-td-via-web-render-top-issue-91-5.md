...
## Step 2b : Auto-resize du Web Render TOP (debounced)

**Problème** : les expressions `parent().width/height` sur `resolutionw/h` changent à chaque pixel pendant le resize, ce qui cause un écran noir (le Chromium perd son état de rendu).

**Solution** : debouncer le resize via un script Python. Ne mettre à jour la résolution qu'après N frames de stabilité, puis pulser `reloadsrc` pour recharger la page.

**Implémentation** :
1. Supprimer les expressions sur `resolutionw`/`resolutionh` → mode CONSTANT
2. Créer un `scriptCHOP` ou utiliser un `timerCHOP` + `chopexecuteDAT` qui :
   - Lit `parent().width` / `parent().height` à chaque frame
   - Compare avec la résolution courante du webrender
   - Si changement détecté, attend 30 frames (~0.5s) de stabilité
   - Puis applique la nouvelle résolution + pulse `reloadsrc`
3. `autorestart = True` en filet de sécurité

**Script `resize_debounce`** (chopexecuteDAT ou textDAT avec run()) :
```python
# Stocké dans storage de /TDPretext
# Chaque frame : vérifier si le COMP a changé de taille
# Si oui, reset le compteur
# Si le compteur atteint 30 sans changement, appliquer la résolution
_last_w = 0
_last_h = 0
_stable_frames = 0
DEBOUNCE_FRAMES = 30
...
def check_resize():
    comp = op('/TDPretext')
    wr = op('/TDPretext/webrender_flow')
    cw, ch = int(comp.width), int(comp.height)
    wrw, wrh = wr.par.resolutionw.eval(), wr.par.resolutionh.eval()
    
    if cw == wrw and ch == wrh:
        return  # déjà sync
    
    # Resize détecté — attendre stabilité
    global _last_w, _last_h, _stable_frames
    if cw != _last_w or ch != _last_h:
        _last_w, _last_h = cw, ch
        _stable_frames = 0
        return
    
    _stable_frames += 1
    if _stable_frames >= DEBOUNCE_FRAMES:
        wr.par.resolutionw.val = cw
        wr.par.resolutionh.val = ch
        wr.par.reloadsrc = True
        _stable_frames = 0
```

**Fichiers modifiés** :
- `webrender_flow` : résolution en CONSTANT, `autorestart=True`
- Nouveau : `resize_debounce` (textDAT avec `run()` frame-driven ou chopexecuteDAT sur un timerCHOP)

## Step 6 : Preset Textstring (physique lettres)

**Diagnostic** : Le switch de DAT fonctionne (HTML minimal OK). Le HTML complet textstring échoue silencieusement — probablement l'import ESM qui bloque le `<script type="module">` entier.

**Fix** : Séparer l'import pretext du code principal. Utiliser un `<script>` non-module pour le physics/render, et un import ESM optionnel juste pour le zig-zag. Si l'import échoue, fallback sur l'ordre séquentiel.

**Approche** :
1. Script principal en `<script>` classique (pas `type="module"`) — contient la physique Verlet, le render Canvas, le drag
2. Import pretext via `<script type="module">` séparé qui expose le zig-zag en global si disponible
3. `initLetters()` utilise le zig-zag si dispo, sinon ordre séquentiel
4. Tester dans le webrender
