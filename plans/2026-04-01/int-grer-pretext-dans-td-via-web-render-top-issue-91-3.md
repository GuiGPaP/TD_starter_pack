...
## Step 2b : Auto-resize du Web Render TOP

Le `webrender_flow` doit suivre les dimensions du COMP parent `/TDPretext` automatiquement.

**Approche** : mettre des expressions Python sur les paramètres `resolutionw` et `resolutionh` :
```python
resolutionw → parent().width
resolutionh → parent().height
```

Cela garantit que si le COMP est redimensionné (ex: en perform mode, ou manuellement), le Web Render suit.

**Fichiers modifiés** : uniquement `webrender_flow` via `update_td_node_parameters` (expressions).
