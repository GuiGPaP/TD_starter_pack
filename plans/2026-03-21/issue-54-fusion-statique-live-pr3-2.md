...
**FusionService returns**: `{ entry: EnrichedOperatorEntry, _meta: EnrichmentMeta }` — `_meta` is a sibling of `entry`, not nested inside it.
**Resource handler wraps** with `version: "1"`: `{ version: "1", entry, _meta }` — the `version` envelope belongs to the handler, not the service.
...
- Add `enrichedStaticOperatorParamSchema`: extends `StaticOperatorParam` with the live-injected fields:
  `{ ...StaticOperatorParam, menuNames?: string[], menuLabels?: string[], min?: number|null, max?: number|null, clampMin?: boolean, clampMax?: boolean, val?: unknown }`
  This is the type used in `payload.parameters` after merge — wider than the static-only shape.
- Add `liveParameterSchema` mirroring the generated `ParameterSchema` interface exactly (all fields optional):
  `{ name?, label?, style?, default?: unknown, val?: unknown, min?: number|null, max?: number|null, clampMin?, clampMax?, menuNames?: string[], menuLabels?: string[], isOP?, readOnly?, page? }`
...
- Add `enrichedOperatorEntrySchema`: extends `operatorEntrySchema` where:
  - `payload.parameters` uses `EnrichedStaticOperatorParam[]` (wider type, with live fields injected)
  - `payload.liveParameters` added as optional `ParameterSchema[]` (full live dump, all fields optional per generated interface)
- Export: `TDKnowledgeEntry` (union), `TDPythonModuleEntry`, `TDOperatorEntry`, `EnrichmentMeta`, `EnrichedOperatorEntry`, `EnrichedStaticOperatorParam`
...
5. **Live lookup** (two-step: discover instance, then fetch schema):
   - **Discovery**: call `tdClient.execPythonScript` with a script following the `scan_script.py` pattern:
     ```python
     _root = op('/project1')
     _children = _root.findChildren(maxDepth=10)
     result = None
     for _c in _children:
         if _c.OPType == '{opType}':
             result = _c.path
     ...
     `maxDepth` is a named constant `DISCOVERY_MAX_DEPTH = 10` in fusionService.ts. Accepts the tradeoff: deeper projects may have instances beyond this depth, but keeps discovery fast. Can be tuned later.
   - If no instance found → return `{ entry, _meta: { source: "static" } }` (graceful fallback)
   - If instance found → call `tdClient.getNodeParameterSchema({ nodePath: foundPath })`
   - If that call fails → return `{ entry, _meta: { source: "static" } }` with warning log
...
- `entry.payload.parameters` = `EnrichedStaticOperatorParam[]` — static params enriched with live values where matched
- `entry.payload.liveParameters` = `ParameterSchema[]` — full live dump, all fields optional per generated interface. This is the complete parameter list from the running build (curated view + raw dump pattern).
...
- **Merge rules**: static `description` preserved, live `style`/`default`/ranges override on matching params; live-only params only in `payload.liveParameters` (NOT appended to `payload.parameters`)
