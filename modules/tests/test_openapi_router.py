...
class TestOpenAPIRouter:
	def test_register_and_route(self):
		...
		route = RouteDefinition(
			method="GET", path_pattern="/test", operation_id="test_op"
		)
		router.routes = [route]
	...
	def test_missing_handler(self):
		router = OpenAPIRouter(load_schema=False)
		route = RouteDefinition(
			method="GET", path_pattern="/test", operation_id="no_handler"
		)
		router.routes = [route]
		result = router.route_request("GET", "/test", {}, None)
		assert result["success"] is False
	...
	def test_handler_error(self, mock_log):
		router = OpenAPIRouter(load_schema=False)
		route = RouteDefinition(
			method="GET", path_pattern="/err", operation_id="err_op"
		)
		router.routes = [route]
	...
	def test_post_body_forwarded(self):
		router = OpenAPIRouter(load_schema=False)
		route = RouteDefinition(
			method="POST", path_pattern="/data", operation_id="post_op"
		)
		router.routes = [route]
	...
	def test_query_params_forwarded(self):
		router = OpenAPIRouter(load_schema=False)
		route = RouteDefinition(
			method="GET", path_pattern="/q", operation_id="q_op"
		)
		router.routes = [route]
