...
def export_subgraph(body: str = None, **kwargs) -> Result:
    """
    Auto-generated handler for operation: export_subgraph
    """
    try:
        print(f"[DEBUG] Handler 'export_subgraph' called with body: {body}, kwargs: {kwargs}")
        service_method = getattr(get_api_service(), "export_subgraph", None)
        if not callable(service_method):
            return error_result("Service method 'export_subgraph' not implemented")
        ...
        return error_result(f"Handler for 'export_subgraph' failed: {str(e)}")
