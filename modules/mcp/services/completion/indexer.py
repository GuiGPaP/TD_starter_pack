...
def _render_builtins(stubs: dict[str, Any], compact: bool) -> str:
	...
	if compact:
		...
		lines.extend(
			f"- `{e['name']}` ({e['type']}): {e['desc']}" for e in td_mod
		)
	...
	if compact:
		for page_name, entries in pars.items():
			...
			lines.append(
				f"- **{page_name}**: {', '.join(f'`{n}`' for n in names)}"
			)
	else:
		for page_name, entries in pars.items():
			lines.append(f"### {page_name}")
			lines.extend(
				f"- `{e['name']}` ({e['type']}): {e['desc']}" for e in entries
			)
			lines.append("")
...
def _render_extensions(
	extensions: dict[str, list[dict]], compact: bool
) -> str:
	lines = ["# Extensions", ""]
	...
	if compact and len(items) > 10:
		lines.append(
			f"- ... ({len(items) - 10} more COMPs with extensions)"
		)

	return "\n".join(lines)


def _render_custom_pars(
	custom_pars: dict[str, list[dict]], compact: bool
) -> str:
	...
	if compact and len(items) > 10:
		lines.append(
			f"- ... ({len(items) - 10} more COMPs with custom parameters)"
		)
